...
async def new_campaign_wizard(core: CodexCore = None) -> Optional[GameSave]:  # type: ignore[assignment]
    ...
    from codex_world_engine to create a custom world.
    """
    console.clear()
    ...
    worlds = [
        ...
        # ("Eberron", "Magitech noir adventure"),  # FILTERED: Not yet implemented
        # ("Ravenloft", "Gothic horror domains"),  # FILTERED: Not yet implemented
        ("Homebrew", "Craft your own world"),
        ("Load World", "Resume a saved Homebrew world"),
    ]

    worlds_table = Table(box=box.ROUNDED, border_style=ROYAL_BLUE, title="[bold]Choose Thy Realm[/bold]")
    ...
    valid_range = [str(n) for n in range(1, len(worlds) + 1)]
    world_choice = await ainput(f"[gold1]Select realm [1-{len(worlds)}]:[/] ")
    if world_choice not in valid_range:
        world_choice = "1"

    world_state_dict: Optional[dict] = None
    choice_idx = int(world_choice)

    if choice_idx == 2:
        # Launch World Genesis Wizard (Homebrew)
        we: WorldEngine = get_world_engine()
        architect = core.architect if core else None
        ws: Optional[WorldState] = await we.run_genesis_wizard(architect=architect)
        if ws is None:
            return None
        world_name = ws.name
        world_state_dict = ws.to_dict()
    elif choice_idx == 3:
        # Browse saved worlds (Load World)
        we: WorldEngine = get_world_engine()
        ws: Optional[WorldState] = await we.browse_worlds()
        if ws is None:
            return None
        world_name = ws.name
        world_state_dict = ws.to_dict()
    else:
        world_name = worlds[choice_idx - 1][0]

    console.print()
    default_name = f"Quest of {datetime.now().strftime('%B')}"
    campaign_name = await ainput(f"[gold1]Name thy campaign[/] [{default_name}]: ")
    if not campaign_name:
        campaign_name = default_name

    if await aconfirm("Forge this campaign?", default=True):
        save = GameSave(
        ...
        # Store world_state in the save for Crown & Crew injection
        if world_state_dict:
            save.state["world_state"] = world_state_dict
        save.save()
        console.print(f"\n[green]Campaign '{campaign_name}' created![/green]")
        await ainput("\n[dim]Press Enter to continue...[/dim]")
        return save
